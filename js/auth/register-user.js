...
function validateRegistration({ email, password, repeatPassword }) {
    ...
    if (password && password.length < MIN_PASSWORD_LENGTH) {
        // throw new Error(`Password should contain at least ${IN_PASSWORD_LENGTH} characters.`);
    ...
    if (password !== repeatPassword) {
        // throw new Erorr('Passwords does not match');
...
export default function registerUser(event) {
    // для того чтоб не перезагружалась страница
    event.preventDefault();

    const formData = new FormData(event.target);
    // в get пишем имя не тип!
